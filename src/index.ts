...
export default (opts?: Partial<Config>) => ({
	FunctionExit: {
		fluid(f: LightningCssFunction) {
			...
			const isLengthValue = (
				val: TokenOrValue,
			): val is { type: "length"; value: LengthValue } => {
			...
			const minSize =
				values[0]?.value?.unit === "rem"
					? values[0].value.value * options.baseFontSize
					: values[0]?.value.value;
			const maxSize =
				values[1]?.value?.unit === "rem"
					? values[1].value.value * options.baseFontSize
					: values[1]?.value.value;

			const minViewPort =
				values[2]?.value?.unit === "px"
					? values[2].value.value
					: options.minViewPort;
			const maxViewPort =
				values[3]?.value?.unit === "px"
